...
class DocumentationValidator:
        ...
        try:
            ...
            if content.startswith('---'):
                try:
                    ...
                    self.errors.append(f"Invalid YAML frontmatter in {file_path}: {e}")
            
            # Basic markdown validation
            try:
                markdown.markdown(content)
            except Exception as e:
                self.errors.append(f"Markdown parsing error in {file_path}: {e}")
            
            # Check for common issues
            self._check_markdown_issues(file_path, content)
            
        except UnicodeDecodeError:
            self.errors.append(f"Cannot read file {file_path}: encoding issue")
        except Exception as e:
    ...
    def _check_markdown_issues(self, file_path: Path, content: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if line.rstrip() != line:
                self.warnings.append(f"Trailing whitespace in {file_path}:{i}")
            
            # Check for tabs instead of spaces
            if '\t' in line:
                self.warnings.append(f"Tab character found in {file_path}:{i}")
            
            # Check for very long lines
            if len(line) > 120:
                self.warnings.append(f"Long line ({len(line)} chars) in {file_path}:{i}")
    ...
    def _validate_internal_links(self):
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            ...
            all_files.add(str(relative_path.with_suffix('')))  # Without .md extension
    ...
    def _validate_internal_link(self, source_file: Path, link: str, all_files: Set[str]):
        ...
        if source_dir == Path('.'):
            ...
            target_path = str((source_dir / link_path).as_posix())
        ...
        if target_path not in all_files and (target_path + '.md') not in all_files:
            ...
            if not full_path.exists() and not (full_path.with_suffix('.md')).exists():
                self.errors.append(f"Broken internal link in {source_file}: {link}")
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            ...
            for match in re.finditer(img_pattern, content):
                ...
                if not full_path.exists():
                    self.errors.append(f"Missing image in {md_file}: {img_path}")
    ...
    def _validate_mkdocs_config(self):
        ...
        if not config_path.exists():
            self.errors.append("mkdocs.yml configuration file not found")
            return
        ...
        try:
            ...
            for field in required_fields:
                if field not in config:
                    self.errors.append(f"Missing required field '{field}' in mkdocs.yml")
            ...
            self.errors.append(f"Error reading mkdocs.yml: {e}")
                ...
                for key, value in item.items():
                    if isinstance(value, str):
                        ...
                        if not nav_file.exists():
                            self.errors.append(f"Navigation references missing file: {value}")
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            try:
                ...
                result = subprocess.run([
                    'aspell', 'list', '--lang=en', '--personal=/dev/null'
                ], input=text_content, capture_output=True, text=True)
    ...
    def _report_results(self, coverage_report: CoverageReport):
        ...
        if self.warnings:
            ...
            for warning in self.warnings:
                location = f"{warning.file_path}:{warning.line_number}" if warning.line_number > 0 else warning.file_path
        ...
        if self.info:
            ...
            for info_item in self.info[:10]:  # Limit output
                location = f"{info_item.file_path}:{info_item.line_number}" if info_item.line_number > 0 else info_item.file_path
