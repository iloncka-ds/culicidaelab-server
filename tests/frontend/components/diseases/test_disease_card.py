...
def test_disease_card_rendering(mock_router, mock_disease_data):
    ...
    with patch('solara.use_router', return_value=mock_router):
        ...
        # Verify the router was set up
        assert disease_card.use_router.called
        
        # Verify the card was created with the correct props
        rv = disease_card.rv
        rv.Card.assert_called_once()
        
        # Check that the image is rendered when image_url is provided
        ...
        # Check that the disease name is rendered
...
def test_disease_card_click_behavior(mock_router, mock_disease_data):
    ...
    with patch('solara.use_router', return_value=mock_router):
        ...
        # Verify the selected disease was updated
...
def test_disease_card_without_image():
    ...
    with patch('solara.use_router'):
        ...
        # Verify the icon is shown when no image is provided
        rv = disease_card.rv
        rv.Icon.assert_called_once()
        
        # The image component should not have been called
...
def test_disease_card_without_prevalence():
    ...
    with patch('solara.use_router'):
        ...
        # The chip component should not have been called since there's no prevalence
        rv = disease_card.rv
