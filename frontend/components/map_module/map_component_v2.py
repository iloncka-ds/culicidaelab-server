...
def MapDisplay():
    ...
    # Keep track of the running task
    ...
    # Effect to trigger data loading with proper cleanup
    solara.use_effect(
        lambda: (
            # Start the task and store it in task_ref.current
            setattr(task_ref, "current", asyncio.create_task(load_all_data())),
            # Return cleanup function that cancels the task if it exists
