...
@solara.component
def Page():
    # from solara.lab.theme import theme
    ...
    species_id: Optional[str] = "aedes_albopictus"
    ...
    if route_current and hasattr(route_current, "path_params") and route_current.path_params:
        ...
        species_id = "aedes_albopictus"
        print(f"DEBUG: SPECIES_DETAIL.PY - Retrieved species_id from route_current.path_params: '{species_id}'")
    elif route_current:
        print(
            f"DEBUG: SPECIES_DETAIL.PY - route_current.path_params is missing or empty. Route: {route_current.path}, Resolved Path: {route_current.resolved_path}"
        )
    else:
        print(f"DEBUG: SPECIES_DETAIL.PY - route_current is None. Cannot get species_id.")
    # State for this page
    ...
    def _fetch_species_detail_effect() -> Optional[Callable[[], None]]:
        ...
        async def _async_task():
            ...
            if not species_id:
                print("DEBUG: SPECIES_DETAIL.PY Effect: No species_id provided (from route_current.path_params).")
    ...
    with solara.Column(align="center", classes=["pa-4"], style="max-width: 900px; margin: auto;"):
        ...
        if loading:
            ...
            solara.Error(f"Could not load species details: {error}", icon="mdi-alert-circle-outline", style="margin-top: 20px;")
        elif not species_id:
            print(
                f"DEBUG: SPECIES_DETAIL.PY - Rendering 'no species ID specified' because species_id is '{species_id}'"
            )
            solara.Info("No species ID specified in the URL.", icon="mdi-information-outline", style="margin-top: 20px;")
